...
config = {
  ...
  'operation': [{'selector': '.method-title', 'split': True}, {'selector': './/*[@class="documentation__routes"]/div/*[@class="section toc-el"][contains(.//dt[contains(text(),"Endpoint format")]/following-sibling::dd[1]/a/text(), "Content-")]', 'type': 'xpath'}],
...
def fixEndPointParameters(parameters, endpoint_format):
    if endpoint_format.lower() in ['content-download', 'content-upload']:
      for x in parameters:
        if x['in'] == 'header':
          x['name'] = 'Dropbox-API-Arg'

    return parameters

def fixOperationData(operation_data):
  if '/files/upload' in operation_data:
    if 'post' in operation_data['/files/upload']:
      consume_data = {'consumes': ["application/octet-stream"]}
      operation_data['/files/upload']['post'].update(consume_data)
